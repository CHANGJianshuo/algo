...
class KVAwareUserNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        # millow_6 Multi-View Pooling: project (3 × emb_dim) -> d_model.
        # Each KV pair produces 3 view vectors (sum, max, mean) concatenated
        # to 3*emb_dim before this Linear. Captures complementary signals:
        #   - sum-pool : V-weighted summary
        #   - max-pool : the K with strongest |V| (top preference)
        #   - mean-pool: unweighted user breadth (which K's even appeared)
    ...
    def forward(
        ...
        # ── KV-Aware path (millow_6: Multi-View Pooling) ─────────
        # Three complementary pools per KV pair, concatenated to 3*emb_dim
        # before projection. More robust than any single pool on the very
        # short V sequences (length 3-10) typical in this dataset.
        for i, (fid, _k_idx, k_offset, k_length, _k_vocab, v_offset, v_length) \
            ...
            # View 1: V-weighted sum-pool, normalized by sum(|V|)
            weighted = (k_emb * v_masked.unsqueeze(-1)).sum(dim=1)              # (B, emb_dim)
            v_abs_sum = v_masked.abs().sum(dim=1, keepdim=True).clamp(min=1e-6)
            sum_view = weighted / v_abs_sum                                     # (B, emb_dim)

            # View 2: max-pool — pick K_emb at position with largest |V|
            v_for_argmax = v_masked.abs().masked_fill(mask == 0, float('-inf'))  # (B, N)
            # Handle all-padded rows (all -inf) by replacing with first index
            no_valid = (mask.sum(dim=1) == 0)                                    # (B,)
            argmax = v_for_argmax.argmax(dim=1)                                  # (B,)
            argmax = torch.where(no_valid, torch.zeros_like(argmax), argmax)
            # Gather K_emb at argmax index per row
            max_view = k_emb.gather(1, argmax.view(B, 1, 1).expand(-1, 1, k_emb.shape[-1])).squeeze(1)
            max_view = max_view * (~no_valid).float().unsqueeze(-1)              # zero out empty rows

            # View 3: mean-pool — unweighted average over valid K_emb
            mask_unsq = mask.unsqueeze(-1)                                       # (B, N, 1)
            mean_view_sum = (k_emb * mask_unsq).sum(dim=1)                       # (B, emb_dim)
            valid_count = mask.sum(dim=1, keepdim=True).clamp(min=1.0)            # (B, 1)
            mean_view = mean_view_sum / valid_count                               # (B, emb_dim)

            # Concatenate 3 views → (B, 3*emb_dim) → Linear → 1 token
            multi_view = torch.cat([sum_view, max_view, mean_view], dim=-1)      # (B, 3*emb_dim)
            token = F.silu(self.kv_token_projs[i](multi_view)).unsqueeze(1)      # (B, 1, d_model)
            tokens.append(token)
