...
_FALLBACK_MODEL_CFG = {
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    # W1 quick-win: included so that train_config.json's value (set when
    # --include_ns_in_classifier was used during training) propagates into
    # the rebuilt PCVRHyFormer. Without this entry, _MODEL_CFG_KEYS would
    # ignore it and rebuild output_proj at the wrong input dim, causing
    # strict load_state_dict to fail.
    'include_ns_in_classifier': False,
    # millow_5: DCN-V2 keys must be here so train_config propagates them
    # into the rebuilt PCVRHyFormer; otherwise the dcn submodule won't be
    # constructed and strict load_state_dict will report missing keys.
    'use_dcn': False,
    'dcn_layers': 2,
}
...
def build_model(
    ...
    # W2 KV-Aware: must pass schema fid info so KVAwareUserNSTokenizer can
    # locate K/V slices when ns_tokenizer_type='kv_aware'. Harmless for the
    # rankmixer / group modes (ctor accepts them as unused defaults).
    user_int_fids = [fid for fid, _, _ in dataset.user_int_schema.entries]
    user_dense_field_specs = list(dataset.user_dense_schema.entries)
