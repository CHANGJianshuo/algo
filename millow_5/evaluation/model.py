...
class KVAwareUserNSTokenizer(nn.Module):
    """KV-Aware User NS Tokenizer (W2 core innovation, PLAN.md 11.1).

    The contest data hides eight key-value-aligned field pairs:
    ``user_int_feats_{fid}`` (key IDs) is element-wise paired with
    ``user_dense_feats_{fid}`` (float weights / scores) for fid in
    ``DEFAULT_KV_PAIR_FIDS``. Baseline RankMixer wastes this structure
    by flattening ``user_dense`` into a single 918-dim vector and
    projecting through a single Linear, throwing away the explicit
    "user prefers category X with score V[X]" signal.

    This tokenizer instead:

      * For each of the 8 KV pairs:
          1. Look up K's embedding table -> (B, N, emb_dim)
          2. Element-wise multiply by V (broadcast on emb_dim axis)
          3. Sum-pool across N, normalize by sum(|V|), clamp to avoid /0
          4. Linear -> SiLU -> 1 KV-aware token (d_model)
      * For all non-KV ``user_int`` fields (~38 of them), fall back to
        a built-in RankMixer-style equal-split tokenizer producing
        ``num_non_kv_tokens`` extra tokens.

    Total user-side NS tokens = num_valid_kv_pairs + num_non_kv_tokens.
    The dense (61=SUM, 87=LMF4Ads) NON-KV fields are still handled by
    PCVRHyFormer's user_dense_proj separately so this class only owns
    the integer + KV-paired-value path.
    """

    DEFAULT_KV_PAIR_FIDS: Tuple[int, ...] = (62, 63, 64, 65, 66, 89, 90, 91)

    def __init__(
        self,
        user_int_feature_specs: List[Tuple[int, int, int]],
        user_int_fids: List[int],
        user_dense_field_specs: List[Tuple[int, int, int]],
        emb_dim: int,
        d_model: int,
        num_non_kv_tokens: int = 5,
        emb_skip_threshold: int = 0,
        kv_pair_fids: Optional[Tuple[int, ...]] = None,
    ) -> None:
        super().__init__()
        self.emb_dim = emb_dim
        self.d_model = d_model

        kv_pair_fids = tuple(kv_pair_fids) if kv_pair_fids else self.DEFAULT_KV_PAIR_FIDS

        if len(user_int_fids) != len(user_int_feature_specs):
            raise ValueError(
                f"user_int_fids len {len(user_int_fids)} != "
                f"user_int_feature_specs len {len(user_int_feature_specs)}")
        user_int_fid_to_idx = {fid: i for i, fid in enumerate(user_int_fids)}
        user_dense_fid_to_offset_len = {
            fid: (offset, length) for fid, offset, length in user_dense_field_specs
        }

        # Build the list of valid KV pairs (both K and V exist, lengths match).
        # Each entry: (fid, k_idx_in_user_int, k_offset, k_length, k_vocab,
        #              v_offset, v_length).
        valid_pairs: List[Tuple[int, int, int, int, int, int, int]] = []
        kv_k_idx_set: set = set()
        for fid in kv_pair_fids:
            if fid not in user_int_fid_to_idx or fid not in user_dense_fid_to_offset_len:
                logging.warning(
                    f"KVAwareUserNSTokenizer: fid {fid} missing from int or dense "
                    f"schema; skipping this pair")
                continue
            k_idx = user_int_fid_to_idx[fid]
            k_vocab, k_offset, k_length = user_int_feature_specs[k_idx]
            v_offset, v_length = user_dense_fid_to_offset_len[fid]
            if k_length != v_length:
                logging.warning(
                    f"KVAwareUserNSTokenizer: fid {fid} K-length {k_length} != "
                    f"V-length {v_length}; skipping (data mis-alignment)")
                continue
            valid_pairs.append(
                (fid, k_idx, k_offset, k_length, k_vocab, v_offset, v_length))
            kv_k_idx_set.add(k_idx)

        self._kv_pair_info = valid_pairs
        self.num_kv_tokens = len(valid_pairs)

        # Per-pair K embedding (skipped if vocab > emb_skip_threshold).
        kv_k_embs_raw: List[Optional[nn.Embedding]] = []
        for _fid, _k_idx, _k_off, _k_len, k_vocab, _v_off, _v_len in valid_pairs:
            skip = int(k_vocab) <= 0 or (
                emb_skip_threshold > 0 and int(k_vocab) > emb_skip_threshold)
            if skip:
                kv_k_embs_raw.append(None)
            else:
                kv_k_embs_raw.append(nn.Embedding(int(k_vocab) + 1, emb_dim, padding_idx=0))
        self.k_embs = nn.ModuleList([e for e in kv_k_embs_raw if e is not None])
        # Map valid_pairs index -> position in self.k_embs (-1 if skipped).
        self._k_emb_index: List[int] = []
        real_idx = 0
        for e in kv_k_embs_raw:
            if e is not None:
                self._k_emb_index.append(real_idx)
                real_idx += 1
            else:
                self._k_emb_index.append(-1)

        # Per-pair output projection (emb_dim -> d_model).
        self.kv_token_projs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(emb_dim, d_model),
                nn.LayerNorm(d_model),
            )
            for _ in valid_pairs
        ])

        # millow_4: reverted to W2 sum-pool (W3 attn pool empirically
        # under-performed: softmax over short V sequences of length 3-10
        # is unstable, and the new MLP params under-train within the 5-epoch
        # early-stop window). No per-pair learnable pool head here.

        # Non-KV user_int path: collect indices not used by any KV pair.
        non_kv_indices = [i for i in range(len(user_int_feature_specs)) if i not in kv_k_idx_set]
        self._non_kv_specs: List[Tuple[int, int, int]] = [
            user_int_feature_specs[i] for i in non_kv_indices
        ]
        self.num_non_kv_tokens = int(num_non_kv_tokens) if non_kv_indices else 0

        # Non-KV embeddings (per-fid).
        non_kv_embs_raw: List[Optional[nn.Embedding]] = []
        for vs, _off, _len in self._non_kv_specs:
            skip = int(vs) <= 0 or (
                emb_skip_threshold > 0 and int(vs) > emb_skip_threshold)
            if skip:
                non_kv_embs_raw.append(None)
            else:
                non_kv_embs_raw.append(nn.Embedding(int(vs) + 1, emb_dim, padding_idx=0))
        self.non_kv_embs = nn.ModuleList([e for e in non_kv_embs_raw if e is not None])
        self._non_kv_emb_index: List[int] = []
        real_idx = 0
        for e in non_kv_embs_raw:
            if e is not None:
                self._non_kv_emb_index.append(real_idx)
                real_idx += 1
            else:
                self._non_kv_emb_index.append(-1)

        # Non-KV equal-split projections (RankMixer-style).
        if self.num_non_kv_tokens > 0:
            total_dim_non_kv = len(self._non_kv_specs) * emb_dim
            self._non_kv_chunk_dim = math.ceil(total_dim_non_kv / self.num_non_kv_tokens)
            self._non_kv_padded_total = self._non_kv_chunk_dim * self.num_non_kv_tokens
            self._non_kv_pad_size = self._non_kv_padded_total - total_dim_non_kv
            self.non_kv_token_projs = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(self._non_kv_chunk_dim, d_model),
                    nn.LayerNorm(d_model),
                )
                for _ in range(self.num_non_kv_tokens)
            ])
        else:
            self._non_kv_chunk_dim = 0
            self._non_kv_padded_total = 0
            self._non_kv_pad_size = 0
            self.non_kv_token_projs = nn.ModuleList()

        self.total_user_ns_tokens = self.num_kv_tokens + self.num_non_kv_tokens

        logging.info(
            f"KVAwareUserNSTokenizer: {self.num_kv_tokens}/"
            f"{len(kv_pair_fids)} valid KV pairs (fids={[p[0] for p in valid_pairs]}), "
            f"{self.num_non_kv_tokens} non-KV RankMixer tokens "
            f"over {len(non_kv_indices)} fields, "
            f"total {self.total_user_ns_tokens} user NS tokens"
        )

    # ── Compatibility shims for PCVRHyFormer's _init_params /
    #    reinit_high_cardinality_params (which iterate `embs` + `feature_specs`
    #    + `_emb_index`). We expose aggregated views so the existing
    #    bookkeeping code Just Works without special-casing this class.

    @property
    def embs(self) -> nn.ModuleList:
        return nn.ModuleList(list(self.k_embs) + list(self.non_kv_embs))

    @property
    def feature_specs(self) -> List[Tuple[int, int, int]]:
        # KV K specs (vocab, k_offset, k_length) followed by non-KV specs.
        kv_specs = [(p[4], p[2], p[3]) for p in self._kv_pair_info]
        return kv_specs + list(self._non_kv_specs)

    @property
    def _emb_index(self) -> List[int]:
        n_k = len(self.k_embs)
        # Non-KV indices need offset since `.embs` puts K embs first.
        non_kv_offset_indices = [
            (i + n_k) if i != -1 else -1 for i in self._non_kv_emb_index
        ]
        return list(self._k_emb_index) + non_kv_offset_indices
    ...
    def forward(
        ...
        """
        Args:
            int_feats:   (B, total_user_int_dim) — full user_int vector
            dense_feats: (B, total_user_dense_dim) — full user_dense vector
        Returns:
            (B, total_user_ns_tokens, d_model)
        """
        B = int_feats.shape[0]
        tokens: List[torch.Tensor] = []

        # ── KV-Aware path (millow_4: W2 sum-pool) ────────────────
        # V-weighted sum of K embeddings, normalized by sum(|V|). Simple
        # but stable on short V sequences (3-10 elements). Verified at
        # AUC 0.8138 (vs baseline 0.8125) — modest +0.0013 lift but
        # positive. The W3 learnable attention pool was empirically worse
        # (-0.0095), most likely because softmax over very short
        # sequences is unstable + the new params don't have time to train.
        for i, (fid, _k_idx, k_offset, k_length, _k_vocab, v_offset, v_length) \
                in enumerate(self._kv_pair_info):
            k_real = self._k_emb_index[i]
            if k_real == -1:
                tokens.append(int_feats.new_zeros(B, 1, self.d_model, dtype=torch.float))
                continue

            k_ids = int_feats[:, k_offset:k_offset + k_length].long()         # (B, N)
            v_vals = dense_feats[:, v_offset:v_offset + v_length].float()      # (B, N)

            k_emb = self.k_embs[k_real](k_ids)                                  # (B, N, emb_dim)
            mask = (k_ids != 0).float()                                         # (B, N)
            v_masked = v_vals * mask                                            # (B, N)

            # V-weighted sum-pool, normalized by sum(|V|) for scale stability.
            weighted = (k_emb * v_masked.unsqueeze(-1)).sum(dim=1)              # (B, emb_dim)
            v_norm = v_masked.abs().sum(dim=1, keepdim=True).clamp(min=1e-6)    # (B, 1)
            pooled = weighted / v_norm                                          # (B, emb_dim)

            token = F.silu(self.kv_token_projs[i](pooled)).unsqueeze(1)         # (B, 1, d_model)
            tokens.append(token)

        # ── Non-KV RankMixer path ─────────────────────────────────
        if self.num_non_kv_tokens > 0:
            non_kv_emb_list: List[torch.Tensor] = []
            for j, (_vs, offset, length) in enumerate(self._non_kv_specs):
                emb_real = self._non_kv_emb_index[j]
                if emb_real == -1:
                    fid_emb = int_feats.new_zeros(B, self.emb_dim, dtype=torch.float)
                else:
                    emb_layer = self.non_kv_embs[emb_real]
                    if length == 1:
                        fid_emb = emb_layer(int_feats[:, offset].long())
                    else:
                        vals = int_feats[:, offset:offset + length].long()
                        emb_all = emb_layer(vals)
                        m = (vals != 0).float().unsqueeze(-1)
                        cnt = m.sum(dim=1).clamp(min=1)
                        fid_emb = (emb_all * m).sum(dim=1) / cnt
                non_kv_emb_list.append(fid_emb)
            cat_emb = torch.cat(non_kv_emb_list, dim=-1)
            if self._non_kv_pad_size > 0:
                cat_emb = F.pad(cat_emb, (0, self._non_kv_pad_size))
            chunks = cat_emb.split(self._non_kv_chunk_dim, dim=-1)
            for chunk, proj in zip(chunks, self.non_kv_token_projs):
...
class CrossNetV2(nn.Module):
    """DCN-V2 cross network (Wang et al., WWW 2021).

    Each layer computes:
        x_{l+1} = x_0 * (W_l @ x_l + b_l) + x_l

    where x_0 is the initial flattened-NS-tokens vector and x_l is the
    layer-l representation. This explicitly models feature crosses up to
    order ``num_layers`` while keeping a residual path so original NS
    information is preserved.

    Used as a side branch on top of NS tokens before they enter the
    HyFormer block stack — empirically a +0.5~2% AUC lever in CTR
    benchmarks (Google production, Avazu, Criteo).
    """

    def __init__(self, dim: int, num_layers: int = 2) -> None:
        super().__init__()
        self.num_layers = int(num_layers)
        self.weights = nn.ModuleList([
            nn.Linear(dim, dim, bias=True) for _ in range(self.num_layers)
        ])
        self.layer_norms = nn.ModuleList([
            nn.LayerNorm(dim) for _ in range(self.num_layers)
        ])

    def forward(self, x0: torch.Tensor) -> torch.Tensor:
        """x0: (B, dim) -> (B, dim)"""
        x = x0
        for w, ln in zip(self.weights, self.layer_norms):
            wx = w(x)
            x = ln(x0 * wx + x)
        return x
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # W1 quick-win: also feed NS tokens into the final classifier
        # (default off keeps the original baseline output dim).
        include_ns_in_classifier: bool = False,
        # W2 KV-Aware Tokenization (PLAN.md 11.1):
        #   - user_int_fids: parallel fid list to user_int_feature_specs,
        #     required when ns_tokenizer_type='kv_aware' so the tokenizer can
        #     identify which entries are KV pair K's.
        #   - user_dense_field_specs: [(fid, offset, length), ...] from
        #     dataset.user_dense_schema.entries; lets the tokenizer locate
        #     the V slice for each KV pair inside the flat user_dense vector.
        user_int_fids: Optional[List[int]] = None,
        user_dense_field_specs: Optional[List[Tuple[int, int, int]]] = None,
        # millow_5 — DCN-V2 cross network on flattened NS tokens.
        use_dcn: bool = False,
        dcn_layers: int = 2,
    ) -> None:
        super().__init__()
        ...
        self.d_model = d_model
        self.include_ns_in_classifier = bool(include_ns_in_classifier)
        ...
        elif ns_tokenizer_type == 'kv_aware':
            # W2 KV-Aware: user side uses KVAwareUserNSTokenizer; item side
            # remains RankMixer (item has no KV-aligned dense features).
            if user_int_fids is None or user_dense_field_specs is None:
                raise ValueError(
                    "ns_tokenizer_type='kv_aware' requires user_int_fids and "
                    "user_dense_field_specs to be provided. Pass them from "
                    "dataset.user_int_schema.entries / user_dense_schema.entries.")
            num_non_kv = user_ns_tokens if user_ns_tokens > 0 else 5
            self.user_ns_tokenizer = KVAwareUserNSTokenizer(
                user_int_feature_specs=user_int_feature_specs,
                user_int_fids=user_int_fids,
                user_dense_field_specs=user_dense_field_specs,
                emb_dim=emb_dim,
                d_model=d_model,
                num_non_kv_tokens=num_non_kv,
                emb_skip_threshold=emb_skip_threshold,
            )
            num_user_ns = self.user_ns_tokenizer.total_user_ns_tokens

            # Item side: keep RankMixer (no item-side KV alignment in the data).
            if item_ns_tokens <= 0:
                item_ns_tokens = len(item_ns_groups)
            self.item_ns_tokenizer = RankMixerNSTokenizer(
                feature_specs=item_int_feature_specs,
        ...
        #
        # In kv_aware mode the V part of each KV pair is already consumed by
        # KVAwareUserNSTokenizer, so feeding V into this projection again
        # would double-count it (the model just learns to ignore one copy).
        # We therefore restrict the projection to NON-KV dense fields
        # (fid 61=SUM, fid 87=LMF4Ads).
        self.has_user_dense = user_dense_dim > 0
        self._user_dense_use_full = True
        self._non_kv_dense_slices: List[Tuple[int, int]] = []
        if self.has_user_dense:
            if (ns_tokenizer_type == 'kv_aware'
                    and user_dense_field_specs is not None):
                kv_fid_set = set(KVAwareUserNSTokenizer.DEFAULT_KV_PAIR_FIDS)
                non_kv_dense = [
                    (fid, off, length)
                    for fid, off, length in user_dense_field_specs
                    if fid not in kv_fid_set
                ]
                non_kv_total_dim = sum(length for _fid, _off, length in non_kv_dense)
                if non_kv_total_dim == 0:
                    self.has_user_dense = False
                    logging.warning(
                        "kv_aware: no non-KV user_dense fields, user_dense_proj disabled")
                else:
                    self._user_dense_use_full = False
                    self._non_kv_dense_slices = [
                        (off, length) for _fid, off, length in non_kv_dense
                    ]
                    self.user_dense_proj = nn.Sequential(
                        nn.Linear(non_kv_total_dim, d_model),
                        nn.LayerNorm(d_model),
                    )
                    logging.info(
                        f"kv_aware: user_dense_proj uses only non-KV fields "
                        f"({len(non_kv_dense)} fields, total_dim={non_kv_total_dim}; "
                        f"V part of KV pairs is consumed by KV-aware tokenizer "
        ...
        # millow_5: DCN-V2 cross network operates on the flattened NS-token
        # vector (B, num_ns * d_model). Output is reshaped back to
        # (B, num_ns, d_model) and added (residual) into the NS tokens
        # before they feed into the HyFormer blocks.
        self.use_dcn = bool(use_dcn)
        if self.use_dcn:
            self.dcn = CrossNetV2(self.num_ns * d_model, num_layers=int(dcn_layers))
        else:
            self.dcn = None
        ...
        if rank_mixer_mode == 'full' and d_model % T != 0:
            if ns_tokenizer_type == 'kv_aware':
                # kv_aware naturally yields T values that don't always divide
                # d_model (e.g. 8 KV + 5 non-KV + 1 dense + 2 item = 16,
                # T = 24 with num_queries=2 but d_model=64 % 24 != 0). Fall
                # back to ffn_only RankMixer rather than raising — token
                # mixing in 'full' is not load-bearing for kv_aware variants.
                logging.warning(
                    f"kv_aware: d_model={d_model} %% T={T} != 0; auto-falling "
                    f"back rank_mixer_mode 'full' -> 'ffn_only' to remove the "
                    f"divisibility constraint")
        ...
        # Output projection: optionally concatenate NS tokens with the per-seq
        # query tokens before pooling (controlled by include_ns_in_classifier).
        n_pool_tokens = num_queries * self.num_sequences
        if self.include_ns_in_classifier:
            n_pool_tokens += self.num_ns
        self.output_proj = nn.Sequential(
            nn.Linear(n_pool_tokens * d_model, d_model),
            nn.LayerNorm(d_model),
        )
    ...
    def _run_multi_seq_blocks(
        ...
        # Output: concatenate all sequences' Q tokens (and optionally NS tokens)
        # then project via MLP. NS-in-classifier provides static profile signals
        # directly to the classifier head instead of relying on RankMixer alone
        # to fuse them through Q.
        B = curr_qs[0].shape[0]
        if self.include_ns_in_classifier:
            all_tokens = torch.cat(curr_qs + [curr_ns], dim=1)  # (B, Nq*S + Nns, D)
        else:
            all_tokens = torch.cat(curr_qs, dim=1)              # (B, Nq*S, D)
        output = all_tokens.view(B, -1)                          # (B, T*D)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # KV-aware tokenizer needs both int and dense feats to look up KV pair
        # values; other tokenizers only need int feats.
        if self.ns_tokenizer_type == 'kv_aware':
            user_ns = self.user_ns_tokenizer(
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            if self._user_dense_use_full:
                _ud_input = inputs.user_dense_feats
            else:
                _ud_input = torch.cat(
                    [inputs.user_dense_feats[:, off:off + length]
                     for off, length in self._non_kv_dense_slices],
                    dim=-1,
                )
            user_dense_tok = F.silu(self.user_dense_proj(_ud_input)).unsqueeze(1)  # (B, 1, D)
        ...
        if self.dcn is not None:
            B_dcn = ns_tokens.shape[0]
            flat = ns_tokens.reshape(B_dcn, -1)              # (B, num_ns*D)
            crossed = self.dcn(flat)                          # (B, num_ns*D)
            ns_tokens = crossed.reshape(B_dcn, self.num_ns, self.d_model)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.has_user_dense:
            ...
            user_dense_tok = F.silu(self.user_dense_proj(_ud_input)).unsqueeze(1)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        if self.has_item_dense:
        ...
        if self.dcn is not None:
            B_dcn = ns_tokens.shape[0]
            flat = ns_tokens.reshape(B_dcn, -1)
            crossed = self.dcn(flat)
            ns_tokens = crossed.reshape(B_dcn, self.num_ns, self.d_model)
