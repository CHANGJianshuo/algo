...
class KVAwareUserNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        # Per-pair attention head: predicts a score for each (K_emb, V_scalar)
        # position; softmax across positions yields attention weights. This
        # replaces the old fixed sum-pool / |V|-normalize, letting the model
        # learn how V should weight K regardless of V's scale (type-A V is in
        # the millions, type-B V is in [-1, 1]). The MLP's first Linear has
        # LayerNorm-equivalent rescaling absorbed into its weights via
        # learning, so no per-pair hardcoded normalization is needed.
        attn_hidden = max(emb_dim // 2, 8)
        self.kv_attn_pools = nn.ModuleList([
            nn.Sequential(
                nn.Linear(emb_dim + 1, attn_hidden),
                nn.SiLU(),
    ...
    def forward(
        ...
        # ── KV-Aware path ─────────────────────────────────────────
        # For each KV pair we run a learned attention pool: a small MLP takes
        # (K_emb_pos, V_pos) at each position and outputs a scalar score; we
        # softmax across positions and weight-sum K_emb. This is robust to V's
        # scale (type-A counts vs type-B [-1,1] cosines) because the MLP can
        # absorb any affine rescaling on V during training.
        for i, (fid, _k_idx, k_offset, k_length, _k_vocab, v_offset, v_length) \
            ...
            # Build (K_emb, V_scalar) features at each position.
            v_feat = (v_vals * mask).unsqueeze(-1)                              # (B, N, 1)
            attn_input = torch.cat([k_emb, v_feat], dim=-1)                    # (B, N, emb_dim+1)
            attn_logits = self.kv_attn_pools[i](attn_input).squeeze(-1)        # (B, N)

            # Mask padding positions to -inf so softmax assigns 0 weight.
            attn_logits = attn_logits.masked_fill(mask == 0, float('-1e9'))

            # Softmax across positions. Handle "no valid position" rows by
            # zeroing the resulting NaN attention weights and the pooled vector.
            attn_weights = F.softmax(attn_logits, dim=1)                       # (B, N)
            attn_weights = torch.nan_to_num(attn_weights, nan=0.0)
            has_valid = (mask.sum(dim=1, keepdim=True) > 0).float()            # (B, 1)

            pooled = (k_emb * attn_weights.unsqueeze(-1)).sum(dim=1) * has_valid  # (B, emb_dim)
