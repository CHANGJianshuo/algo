...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--ns_tokenizer_type', type=str, default='rankmixer',
                        choices=['group', 'rankmixer', 'kv_aware'],
                        help='NS tokenizer variant: '
                             ...
                             'kv_aware = W2 tokenizer that explicitly couples 8 KV-aligned '
                             'user_int/user_dense field pairs (fid 62-66, 89-91) into one '
                             'V-weighted token each, plus rankmixer over the remaining 38 '
                             'non-KV user_int fields. Item side stays rankmixer.')
    ...
    # ──── W1 quick-win flags (off by default, equivalent to original baseline) ────
    parser.add_argument('--use_bf16', action='store_true', default=False,
                        help='Enable bfloat16 mixed-precision for forward+loss '
                             '(backward stays fp32). Adagrad sparse path is unaffected.')
    parser.add_argument('--use_ema', action='store_true', default=False,
                        help='Enable Exponential Moving Average of model weights. '
                             'Validation and saved checkpoints reflect EMA weights. '
                             'The only legal pseudo-ensemble allowed by contest rules.')
    parser.add_argument('--ema_decay', type=float, default=0.999,
                        help='EMA decay factor: shadow = decay * shadow + (1-decay) * model')
    parser.add_argument('--ema_dense_only', action='store_true', default=False,
                        help='EMA only the non-Embedding (dense backbone) parameters. '
                             'Saves ~half memory at the cost of EMA-ing only the dense net.')
    parser.add_argument('--use_lr_warmup', action='store_true', default=False,
                        help='Enable linear warmup (and optional cosine decay) for dense AdamW. '
                             'Sparse Adagrad lr stays fixed.')
    parser.add_argument('--warmup_steps', type=int, default=1000,
                        help='Linear-warmup duration in steps. Effective only with --use_lr_warmup.')
    parser.add_argument('--lr_decay_steps', type=int, default=0,
                        help='Cosine-decay duration in steps after warmup. '
                             '0 = stay at peak lr (warmup-only).')
    parser.add_argument('--lr_min_ratio', type=float, default=0.1,
                        help='Cosine-decay floor as a ratio of peak lr (e.g. 0.1 -> 10%% of base lr).')
    parser.add_argument('--include_ns_in_classifier', action='store_true', default=False,
    ...
    # millow_4 does NOT use DCN — these flags are accepted (and ignored)
    # only so a run.sh that leaks from millow_5 won't crash with
    # "unrecognized arguments" on the platform. They're never forwarded
    # to model_args, so millow_4's model is unaffected.
    parser.add_argument('--use_dcn', action='store_true', default=False,
                        help='[ignored in millow_4] DCN-V2 flag — only used in millow_5')
    parser.add_argument('--dcn_layers', type=int, default=2,
                        help='[ignored in millow_4] DCN layer count — only used in millow_5')
    ...
    parser.add_argument('--no_measure_latency', dest='measure_latency', action='store_false',
                        help='Disable post-training latency measurement.')
    parser.add_argument('--latency_warmup', type=int, default=50,
                        help='Number of warmup iterations before latency measurement.')
    parser.add_argument('--latency_iters', type=int, default=200,
                        help='Number of timed iterations for latency P50/P95/P99.')
...
def main() -> None:
    ...
    model_args = {
        ...
        "include_ns_in_classifier": args.include_ns_in_classifier,
        # W2 KV-Aware: pass schema info so KVAwareUserNSTokenizer can locate
        # K/V slices. Harmless for non-kv_aware modes (ignored by ctor).
        "user_int_fids": [fid for fid, _, _ in pcvr_dataset.user_int_schema.entries],
        "user_dense_field_specs": list(pcvr_dataset.user_dense_schema.entries),
    }
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        ema_dense_only=args.ema_dense_only,
